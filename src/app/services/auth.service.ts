...
const HARDCODED_USERS: HardcodedCredential[] = [
    {
        ...
        user: { id: '1', username: 'admin_erp', name: 'Admin ERP', email: 'admin@erp.com', role: 'admin' },
    },
    {
        email: 'medium@erp.com',
        password: 'Medium@Secure1',
        user: { id: '3', username: 'medium_erp', name: 'Medium User', email: 'medium@erp.com', role: 'medium' },
    },
    {
        email: 'cesar@erp.com',
        password: 'Cesar@Secure1',
        user: { id: '2', username: 'cesar_ramirez', name: 'César Ramírez', email: 'cesar@erp.com', role: 'user' },
    },
];
...
export class AuthService {
    ...
    login(credentials: LoginRequest): Observable<ApiResponse<User>> {
        ...
        if (match) {
            this.authState$.next({ user: match.user, isAuthenticated: true, token: `token-${match.user.id}-${Date.now()}` });
            return of<ApiResponse<User>>({ success: true, message: 'Login exitoso.', data: match.user });
        }
